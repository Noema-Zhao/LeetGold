...
type Lc15Frame = { nums: number[]; i: number | null; l: number | null; r: number | null; total: number | null; ans: number[][]; line: number; message: string };
...
const MEANINGS: Record<string,string> = {
  right:"滑动窗口的右边界，也是当前遍历到的下标", ch:"当前由字符串中取出的字符", last:"哈希表：记录每个字符最近一次出现的下标", left:"当前合法滑动窗口的左边界", ans:"保存已经找到的答案或当前最优结果", l:"当前双指针搜索区间的左指针", r:"当前双指针搜索区间的右指针",
...
const LC15_FRAMES: Lc15Frame[] = [
  {nums:[-1,0,1,2,-1,-4],i:null,l:null,r:null,total:null,ans:[],line:2,message:"示例输入 nums = [-1, 0, 1, 2, -1, -4]。先排序，才能用双指针根据和的大小移动。"},
  {nums:[-4,-1,-1,0,1,2],i:null,l:null,r:null,total:null,ans:[],line:2,message:"排序后得到 [-4, -1, -1, 0, 1, 2]。"},
  {nums:[-4,-1,-1,0,1,2],i:null,l:null,r:null,total:null,ans:[],line:3,message:"初始化 ans=[]，数组长度 n=6。"},
  {nums:[-4,-1,-1,0,1,2],i:0,l:null,r:null,total:null,ans:[],line:4,message:"固定 i=0，nums[i]=-4，再在右侧区间寻找另外两个数。"},
  {nums:[-4,-1,-1,0,1,2],i:0,l:1,r:5,total:null,ans:[],line:9,message:"令 l=1、r=5，双指针从区间两端开始。"},
  {nums:[-4,-1,-1,0,1,2],i:0,l:1,r:5,total:-3,ans:[],line:11,message:"total=-4+(-1)+2=-3，小于 0，需要增大总和。"},
  {nums:[-4,-1,-1,0,1,2],i:0,l:2,r:5,total:null,ans:[],line:13,message:"l 右移到 2；继续计算仍为 -3。"},
  {nums:[-4,-1,-1,0,1,2],i:0,l:3,r:5,total:-2,ans:[],line:13,message:"总和仍小于 0，l 继续右移到 3，此时 total=-2。"},
  {nums:[-4,-1,-1,0,1,2],i:0,l:4,r:5,total:-1,ans:[],line:13,message:"总和仍小于 0，l 移到 4；随后 l 与 r 相遇，本轮结束。"},
  {nums:[-4,-1,-1,0,1,2],i:1,l:null,r:null,total:null,ans:[],line:4,message:"固定 i=1，nums[i]=-1。它与前一个固定值不同，可以继续搜索。"},
  {nums:[-4,-1,-1,0,1,2],i:1,l:2,r:5,total:0,ans:[],line:11,message:"l=2、r=5，total=-1+(-1)+2=0，找到一个答案。"},
  {nums:[-4,-1,-1,0,1,2],i:1,l:2,r:5,total:0,ans:[[-1,-1,2]],line:17,message:"把 [-1, -1, 2] 加入 ans。"},
  {nums:[-4,-1,-1,0,1,2],i:1,l:3,r:4,total:null,ans:[[-1,-1,2]],line:19,message:"找到答案后，l 右移、r 左移，变为 l=3、r=4。"},
  {nums:[-4,-1,-1,0,1,2],i:1,l:3,r:4,total:0,ans:[[-1,-1,2]],line:11,message:"再次计算：total=-1+0+1=0。"},
  {nums:[-4,-1,-1,0,1,2],i:1,l:3,r:4,total:0,ans:[[-1,-1,2],[-1,0,1]],line:17,message:"把 [-1, 0, 1] 加入 ans。"},
  {nums:[-4,-1,-1,0,1,2],i:1,l:4,r:3,total:null,ans:[[-1,-1,2],[-1,0,1]],line:19,message:"l=4、r=3，双指针交错，本轮搜索结束。"},
  {nums:[-4,-1,-1,0,1,2],i:2,l:null,r:null,total:null,ans:[[-1,-1,2],[-1,0,1]],line:5,message:"i=2 时 nums[i] 与 nums[i-1] 都是 -1；跳过，防止产生重复三元组。"},
  {nums:[-4,-1,-1,0,1,2],i:3,l:4,r:5,total:3,ans:[[-1,-1,2],[-1,0,1]],line:11,message:"i=3 时固定 0，l=4、r=5，total=3；r 左移后区间结束。"},
  {nums:[-4,-1,-1,0,1,2],i:null,l:null,r:null,total:null,ans:[[-1,-1,2],[-1,0,1]],line:22,message:"遍历完成，返回 [[-1, -1, 2], [-1, 0, 1]]。"},
];
...
function Lc15Visual({step}:{step:number}) {
  const f=LC15_FRAMES[Math.min(step,LC15_FRAMES.length-1)];
...
export default function Home(){
  ...
  const demoLength=problem?.id==="3"?LC3_FRAMES.length:problem?.id==="15"?LC15_FRAMES.length:Math.min(8,Math.max(1,codeLines.length));
  ...
  const activeCodeLine=problem?.id==="3"?LC3_FRAMES[Math.min(demoStep,LC3_FRAMES.length-1)].line:problem?.id==="15"?LC15_FRAMES[Math.min(demoStep,LC15_FRAMES.length-1)].line:demoStep%Math.max(1,codeLines.length);
  const orderLines=useMemo(()=>codeLines.map((line,index)=>({id:`${index}:${line}`,line})),[codeLines]);
  ...
  const checkOrder=()=>{const ok=orderAnswer.length===orderLines.length&&orderAnswer.every((id,i)=>id===orderLines[i].id);ok?(setFeedback({kind:"correct"}),playSound("correct",soundOn)):wrong(orderLines.map(({line})=>line).join("\n"))};
  ...
  return <main className="app-shell"><Header coins={coins} streak={Math.max(1,Math.ceil(completedProblems/3))} soundOn={soundOn} onSound={()=>setSoundOn(x=>!x)}/>
    ...
    :problem&&<section className="lesson-content"><span className="lesson-kicker">TOP {problem.rank} · {difficultyLabel(problem.difficulty)} · 频次 {problem.frequency}</span><h2>LC {problem.id} · {problem.title}</h2>{stage<4&&<div className="stage-chips"><span className={stage===0?"active":""}>题解演示</span><span className={stage===1?"active":""}>代码排序</span><span className={stage===2?"active":""}>变量含义</span><span className={stage===3?"active":""}>代码挖矿</span></div>}
      {stage===0&&<><Statement problem={problem}/><h3>逐行运行演示</h3><p className="hint">每一步都同步展示变量、数据结构与被执行的代码行。</p><DemoVisual problem={problem} step={demoStep}/><div className="code-card">{codeLines.map((line,i)=><code className={i===activeCodeLine?"active":""} key={i}><em>{i+1}</em>{line}</code>)}</div><div className="demo-controls"><button onClick={()=>setDemoStep(x=>Math.max(0,x-1))}>←</button><button className="play" onClick={()=>setPlaying(x=>!x)}>{playing?"暂停":"▶ 播放"}</button><button onClick={()=>setDemoStep(x=>(x+1)%demoLength)}>→</button></div><div className="narration"><b>步骤 {demoStep+1}/{demoLength}</b><p>{problem.id==="3"?LC3_FRAMES[demoStep].message:problem.id==="15"?LC15_FRAMES[demoStep].message:explainLine(codeLines[activeCodeLine],problem.topic)}</p></div><button className="primary sticky-action" onClick={nextStage}>我看懂了，开始挖矿 <span>→</span></button></>}
      {stage===1&&<><h3>把整段代码排回正确顺序</h3><p className="hint">本关包含题解中的所有非空代码行（含 class 与 def）。答错会显示正确顺序，但必须重新排列并答对才能继续。</p><div className="answer-slots">{orderAnswer.length?orderAnswer.map((id,i)=>{const item=orderLines.find(x=>x.id===id);return <button disabled={feedback.kind!=="idle"} onClick={()=>setOrderAnswer(v=>v.filter((_,j)=>j!==i))} key={id}><em>{i+1}</em><code>{item?.line}</code></button>}):<p>从下面依次选择代码行…</p>}</div><div className="line-bank">{shuffledLines.filter(item=>!orderAnswer.includes(item.id)).map(item=><button disabled={feedback.kind!=="idle"} onClick={()=>setOrderAnswer(v=>[...v,item.id])} key={item.id}><code>{item.line}</code></button>)}</div><Feedback state={feedback} onRetry={()=>retry(()=>setOrderAnswer([]))}/>{feedback.kind!=="wrong"&&<button className="primary sticky-action" onClick={feedback.kind==="correct"?nextStage:checkOrder}>{feedback.kind==="correct"?"进入变量训练":"检查顺序"}<span>→</span></button>}</>}
